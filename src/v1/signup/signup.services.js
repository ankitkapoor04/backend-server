...
const isUserEmailExist = async (email) => {
  try {
    const [result] = await pool.query('SELECT * FROM ticketGeneratorUsers WHERE email = ?', [email]);
    return result;
  } catch (error) {
...
const isUserContactNumberExist = async (contactNumber) => {
  try {
    const [result] = await pool.query('SELECT * FROM ticketGeneratorUsers WHERE contactNumber = ?', [contactNumber]);
    return result;
  } catch (error) {
    throw Boom.badImplementation(error.message);
  }
...
const registerUser = async ({ username, email, password, contactNumber }) => {
  try {
    ...
    const query = 'INSERT INTO ticketGeneratorUsers (username,email, password, contactNumber) VALUES (?, ?, ?, ?)';
